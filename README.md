...
グループ・お気に入り・最近使用の仮想グループによるプリセット管理、タブごとに構成を切り替えられるエフェクトタブ管理、Git ライクなスナップショット履歴とブランチ管理、CRC32 検証付きアトミックファイル書き込みおよび SHA-256 ハッシュによる多重冗長永続化、プリセットパッケージの書き出し・読み込み、クリップボードを利用したコピー・カット・ペースト、正規表現対応の多軸検索、8 言語対応の UI を搭載しています。
...
4. [主な機能](#主な機能)
   ...
   - [プリセット管理](#3-プリセット管理)
   - [仮想グループ](#4-仮想グループ)
   - [スナップショット履歴・ブランチ管理](#5-スナップショット履歴ブランチ管理)
   - [安全なファイル永続化](#6-安全なファイル永続化)
   - [プリセットの書き出し・読み込み](#7-プリセットの書き出し読み込み)
   - [クリップボード連携（コピー・カット・ペースト）](#8-クリップボード連携コピーカットペースト)
   - [多軸プリセット検索](#9-多軸プリセット検索)
   - [レスポンシブ UI](#10-レスポンシブ-ui)
   - [多言語対応](#11-多言語対応)
5. [パラメータ一覧](#パラメータ一覧)
...
- **プリセット検索**: キーワードで表示中のプリセットをリアルタイムに絞り込めます（詳細は [多軸プリセット検索](#9-多軸プリセット検索) 参照）。
- **無効エフェクトの一括削除**: コンテキストメニューから、無効化（チェックオフ）されているエフェクトをまとめて削除できます。
- **履歴管理**: プリセットごとに履歴管理ウィンドウを開くことができます（詳細は [スナップショット履歴・ブランチ管理](#5-スナップショット履歴ブランチ管理) 参照）。
...
1. 「適用中エフェクト」上部の「＋」ボタンを押してタブを追加します。
2. 各タブで異なるエフェクト構成を作成します。
3. タブ名をダブルクリックして用途に合わせた名前に変更します。
4. タブをクリックして切り替えると、選択中タブの構成が即座に反映されます。
 
### プリセットを保存・呼び出す
 
1. エフェクトリストに任意のエフェクトを追加・設定します。
2. 必要に応じてタブを追加し、タブごとにエフェクト構成を作成します。
3. プリセット管理パネルで「追加（＋）」ボタンをクリックし、名前を入力してプリセットを保存します。
4. 別のアイテムに同じ構成を適用したい場合は、プリセットを選択して「適用（✓）」ボタンをクリックします。
